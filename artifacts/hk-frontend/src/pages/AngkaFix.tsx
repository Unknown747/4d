...
function BBCampuran() {
  ...
  const { data, isLoading, isFetching, error, refetch } = useQuery<BBCampuranData>({
    queryKey: ['bb-campuran', digitsParam, autoMode, market],
    queryFn: () => fetchApi<BBCampuranData>(`/bb-campuran${digitsParam ? `?digits=${digitsParam}&market=${market}` : `?market=${market}`}`),
    staleTime: 30_000,
    enabled,
  });
  ...
  function toggleDigit(d: number) {
    ...
    setSelectedDigits(prev => {
      ...
      if (next.has(d)) next.delete(d); else if (next.size < 9) next.add(d);
      return next;
    });
  }
  ...
  return (
    <div className="card">
      ...
      <div className="flex gap-2 mb-4 flex-wrap">
        <button onClick={() => { setAutoMode(true); setSelectedDigits(new Set()); }}
        ...
        <div className="mb-4">
          ...
          <div className="flex flex-wrap gap-2 mb-2">
            {Array.from({ length: 10 }, (_, d) => (
              <button key={d} onClick={() => toggleDigit(d)}
          ...
          <div className="text-xs text-slate-500">
            ...
            {selectedDigits.size >= 2 ? ` → ${Math.pow(selectedDigits.size, 4).toLocaleString()} kombinasi` : ' (min. 2 digit)'}
          </div>
        </div>
      )}

      {!enabled ? (
        <div className="text-center py-8 text-slate-500 text-sm">Pilih minimal 2 digit untuk generate kombinasi</div>
        ...
        <>
          <div className="flex flex-wrap items-center gap-4 mb-4 p-3 bg-[#1a2235] rounded-lg text-xs text-slate-400 border border-[#1e2d45]">
            <span>Digit aktif:&nbsp;{data.activeDigits.map(d => <strong key={d} className="text-amber-400 mr-1">{d}</strong>)}</span>
                ...
                {data.predictions.map((p, i) => (
                  <tr key={p.number} className={`border-b border-slate-800/50 hover:bg-white/3 transition-colors ${i === 0 ? 'bg-amber-500/5' : ''}`}>
                    ...
                    <td className="text-center py-2 px-3">
                      <span className={`font-black font-mono text-base tracking-widest ${i === 0 ? 'text-amber-400' : 'text-white'}`}>{p.number}</span>
                    ...
                    <td className="py-2 px-3">
                      <div className="flex items-center gap-2">
                        <div className="w-16 h-1.5 bg-slate-700 rounded-full overflow-hidden">
                          <div className="h-full bg-amber-400 rounded-full transition-all duration-700" style={{ width: `${Math.min(p.score, 100)}%` }} />
                        </div>
          ...
          {!autoMode && (
            <button onClick={() => refetch()} disabled={isFetching} className="mt-3 btn-ghost text-xs">🔄 Hitung Ulang</button>
          )}
        </>
      ) : null}
    </div>
...
export default function AngkaFix() {
  ...
  const { data, isLoading, error, refetch, isFetching } = useQuery<FixData>({
    queryKey: ['angka-fix', market],
    queryFn: () => fetchApi<FixData>(`/angka-fix?market=${market}`),
    staleTime: 30_000,
  });
  ...
  return (
    <div className="space-y-4">
      <div className="bg-gradient-to-br from-slate-900 to-blue-950 border border-blue-900/40 rounded-xl p-6 text-center">
        ...
        <div className="text-slate-500 text-xs uppercase tracking-widest mb-1">{mi.flag} {mi.short} · Gabungan: Statistik × Shio × Pola Ikutan</div>

        {isLoading ? <div className="flex justify-center py-8"><div className="spinner" /></div>
          : error ? <div className="text-red-400 py-8">{String(error)}</div>
          : data ? (
            ...
            {TYPES.map(t => {
              ...
              return (
                <div key={t} className={`rounded-xl border p-4 text-center transition-all hover:scale-[1.02] ${isBB ? 'border-amber-500/50 bg-amber-950/20' : 'border-slate-700 bg-slate-800/60'}`}>
                  ...
                  <div className={`font-black tracking-widest text-3xl mb-1 ${isBB ? 'text-amber-400' : 'text-white'}`}>{entry.number}</div>
                  ...
                  <div className="bg-slate-700 rounded-full h-1 overflow-hidden mb-1">
                    <div className={`h-full rounded-full transition-all duration-700 ${isBB ? 'bg-gradient-to-r from-amber-400 to-orange-400' : 'bg-gradient-to-r from-blue-400 to-amber-400'}`}
        ...
        <button onClick={() => refetch()} disabled={isFetching} className="mt-6 btn-primary">
