...
// Check if binary exists AND loads correctly (V8 ABI match)
let needsRebuild = !existsSync(BINARY);
if (!needsRebuild) {
  try {
    ...
    console.log('[native] better-sqlite3 binary exists but is stale — rebuilding...');
    needsRebuild = true;
  }
}

if (needsRebuild) {
  console.log('[native] rebuilding better-sqlite3 from source...');
